...
from posts.schemas import Post, PostCreate, CommentCreate, PostUpdate, CommentUpdate
from posts.crud import get_posts, get_post, create_post, create_comment, delete_post, delete_comment, update_post_crud, \
    update_comment_crud
...
async def create_new_post(post: PostCreate, db: AsyncSession = Depends(database.get_db)):
    return await create_post(db=db, post=post)


@router.get("/posts/", response_model=List[Post])
async def read_posts(db: AsyncSession = Depends(database.get_db)):
...
async def update_post(post_id: int, post_update: PostUpdate, db: AsyncSession = Depends(database.get_db)):
...
async def remove_post(post_id: int, db: AsyncSession = Depends(database.get_db)):
...
async def create_new_comment(post_id: int, comment: CommentCreate, db: AsyncSession = Depends(database.get_db)):
    return await create_comment(db=db, post_id=post_id, comment=comment)


@router.delete("/comments/{comment_id}", status_code=204)
async def remove_comment(comment_id: int, db: AsyncSession = Depends(database.get_db)):
...
async def update_comment(comment_id: int, comment_update: CommentUpdate, db: AsyncSession = Depends(database.get_db)):
